...
(async () => {
  ...
  await fs.writeFile("attention-deficithyperactivity-disorder-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("attention-deficithyperactivity-disorder-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
